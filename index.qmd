...
Let's load data in for both and compare.
...
enso_jjas |>
  filter(phasexrx_both %in% c("elnio", "lanina")) |>
...
```{r}
#| label: vis-cropyield-peninsular-ricemaize
...
```{r}
#| label: rainfall-crops-peninsular-kharif
india_regions_all_resid |>
  filter(
    rainfall_region == "Peninsular India",
...
```{r}
#| label: import-temperature-global

here( "data", "berkeley-monthly-tavg-global.txt") |>
  read_table(
    col_names = c(
      "year", "month",
...
#| label: smooth-nino34
nino34 |>
  mutate(nino34_3mth = (nino34 + lead(nino34) + lag(nino34)) / 3) ->
nino34_smooth
```

```{r}
#| label: join-temperature-enso
temp_monthly |>
  inner_join(nino34_smooth, join_by(year == year, month == month)) |>
  mutate(
...
ggplot(temp_toplot) +
  aes(x = date) +
  geom_linerange(
    aes(ymin = anom_annual, ymax = anom_5yr, colour = phase_nino34)) +
  geom_line(aes(y = anom_5yr), colour = "black") +
  geom_point(
    aes(y = anom_annual, colour = phase_nino34),
    data = temp_toplot |> filter(is.na(anom_5yr), !is.na(anom_annual)),
    size = 0.25) +
  scale_colour_manual(values = enso_colours) +
  theme_minimal()
